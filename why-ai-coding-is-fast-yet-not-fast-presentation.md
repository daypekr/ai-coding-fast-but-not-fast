...
## 2. AI 코딩 7~8개월 후, 내 작업 패턴의 변화

- **소스코드를 직접 손으로 만지는 일은 한 달에 두세 번** 정도로 줄었다.
- "내가 직접 짜야지" 마음먹어도 잘 안 된다. 이유:
  - 한 기능이 여러 파일에 흩어져 있고, 파일을 찾고 → 열고 → 읽고 → 수정 위치 찾는 단계가 누적된다.
  - DB 스키마 확인까지 끼면 도구 전환 비용이 더 커진다.
  - 한 번의 자연어 명령으로 이 모든 잡일을 AI가 처리해 준다는 사실을 **이미 알고 있기 때문에** 손코딩이 답답해진다.
- **결론**: 인간 본능(최소 노력 추구)에 따라 AI에 맡기는 흐름은 자연스럽고 되돌릴 수 없다.
